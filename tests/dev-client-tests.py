...
def test_02_get_resource(cloud, client, websocket, api_key):
    ...
    async_response = websocket.wait_for_async_response(async_response_id=async_id, timeout=CALLBACK_TIMEOUT,
                                                       assert_errors=True)

    assert async_response['status'] == 200, 'Invalid response status received from the device!'
    global LIFETIME
...
def test_03_put_resource(cloud, client, websocket, api_key):
    ...
    async_id = put_async_device_request(cloud, RESOURCE_PATH, client.endpoint_id(), str(new_lifetime), headers)
    log.info('Put value "{}" to resource "{}". Async-id: "{}"'.format(new_lifetime, RESOURCE_PATH, async_id))
...
def test_04_subscribe_resource(cloud, client, websocket, api_key):
    headers = {'Authorization': 'Bearer {}'.format(api_key)}
    r = cloud.connect.set_subscription_for_resource(client.endpoint_id(), RESOURCE_PATH, headers,
                                                    expected_status_code=[200, 202])
    if r.status_code == 202:
        async_id = r.json()['async-response-id']
        log.info('Set subscription for resource "{}". Async-id: "{}"'.format(RESOURCE_PATH, async_id))
        async_response = websocket.wait_for_async_response(async_response_id=async_id, timeout=CALLBACK_TIMEOUT,
                                                           assert_errors=True)
        assert async_response['status'] == 200, 'Invalid subscription status received from the device!'

    cloud.connect.get_subscription_status(client.endpoint_id(), RESOURCE_PATH, headers, expected_status_code=200)
    ...
    websocket.wait_for_notification(client.endpoint_id(), RESOURCE_PATH, new_lifetime, timeout=CALLBACK_TIMEOUT,
                                    assert_errors=True)
...
def test_05_factory_reset(cloud, client, websocket, api_key):
    ...
    if device_resource_exists(cloud, FACTORY_RESET_PATH, endpoint_id, headers):
        ...
        async_response = websocket.wait_for_async_response(async_response_id=async_id, timeout=CALLBACK_TIMEOUT,
                                                           assert_errors=True)
        assert async_response['decoded_payload'] == '', 'Invalid payload received from the device!'
    else:
